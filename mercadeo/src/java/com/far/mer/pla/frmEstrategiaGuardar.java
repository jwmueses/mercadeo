 ...
 * @author Jorge
 */
public class frmEstrategiaGuardar extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            String id = request.getParameter("id_estrategia");
            ...
            if(!objEstrategia.hayEstrategia(id, id_plan_mercadeo, estrategia)){
                if(id.compareTo("-1")==0){
                    String pk = objEstrategia.ingresar(id_plan_mercadeo, estrategia, concepto, tactica);
                    if(pk.compareTo("-1")!=0){
                        res = "obj»axEstrategias^fun»mer_estrategiaEditar("+pk+")^msg»Información guardada satisfactoriamente.^frm»"+this.getEstrategias(objEstrategia, id_plan_mercadeo);
                    }else{
                        res = "msg»" + objEstrategia.getError();
                    }
                }else{
                    if(objEstrategia.actualizar(id, estrategia, concepto, tactica)){
                        res = "obj»axEstrategias^msg»Información guardada satisfactoriamente.^frm»"+this.getEstrategias(objEstrategia, id_plan_mercadeo);
                    }else{
                            res = "msg»" + objEstrategia.getError();
                        }
                }
            }else{
                res = "msg»Cambie de nombre de la estrategia ya que el nombre se encuentra registrado.";
            }
        } catch(Exception e) {
    ...
    public String getEstrategias(Estrategia objEstrategia, String id_plan_mercadeo)
    {
        ...
        try{
            ...
            html += "<tr><td colspan=\"3\" align=\"right\"><input type=\"hidden\" id=\"sum_estgs\" value=\""+sumatoria+"\" />"+
