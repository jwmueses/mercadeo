 ...
 * @author Jorge
 */
public class getFarmacia extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try{
            ResultSet rs = objDB.consulta("select top(20) Oficina, Nombre from Oficina with (nolock) where lower(Nombre) like '%"+txt+"%'");
            msg = objDB.getError();
            String Oficina = "";
            String Nombre = "";
            while(rs.next()){
                Oficina = rs.getString("Oficina")!=null?rs.getString("Oficina"):"";
                Nombre = rs.getString("Nombre")!=null?rs.getString("Nombre"):"";
                html += "<option value=\"" + Oficina+"\">" + Nombre+"</option>";
            }
            rs.close();
        }catch(Exception e){
            msg = e.getMessage();
        ...
        try {
            ...
            out.print("<select id=\"farIna\" size=\"5\" style=\"width:222px;\" onclick=\""+onClick+"\">"+html+"</select>");
        } finally {
            out.close();
        }
    }
