 ...
 * @author Jorge
 */
public class getCmbCiudades extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try{
            rsCiudades = objEasy.consulta("select ciudad, descripcion from tbl_ciudad with (nolock) where provincia='"+zona+"' order by descripcion");
            html = DatosDinamicos.combo(rsCiudades, "idCiud", "", "", " TODAS ", 130);
        }catch(Exception e){
            msg = e.getMessage();
        }finally {
            objEasy.cerrar();
        ...
        try {
            out.print("obj»axCiudades^msg»"+msg+"^frm»" + html);
        } finally {
            out.close();
        }
    }
