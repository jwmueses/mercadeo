 ...
 * @author Jorge
 */
public class getProveedores extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String busquedaPor = request.getParameter("por");
        String msg = "";
        String html = "";
        BaseDatos objDB = new BaseDatos(this.easy_ip, this.easy_puerto, this.easy_db, this.easy_usuario, this.easy_clave);
        try{
            
            ResultSet rs = objDB.consulta("select top(20) numero_idproveedor, nombre_comercial from tbl_proveedores with (nolock) "
                    + "where lower(numero_idproveedor) like '"+txt+"%' or lower(nombre_comercial) like '%"+txt+"%'");
            msg = objDB.getError();
            ...
            while(rs.next()){
                ...
                if(busquedaPor.compareTo("ruc")==0){
                    nombre_comercial = numero_idproveedor + " - " + nombre_comercial;
                }
        ...
        try {
            ...
            out.print("<select id=\"prov\" size=\"5\" onclick=\""+onClick+"\">"+html+"</select>");
        } finally {
            out.close();
        }
    }
